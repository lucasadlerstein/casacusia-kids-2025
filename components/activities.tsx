...
import { Palette, Shirt, Drum, Target, BellIcon as Balloon, Wand2, Mic, Ear } from "lucide-react"
...
export function Activities() {
  return (
    <section id="actividades" className="py-16 md:py-24 bg-cream">
      <div className="container mx-auto px-4">
        <h2 className="text-3xl md:text-5xl font-bold text-deep-blue text-center mb-16">Actividades y espacios</h2>

        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {activities.map((activity, index) => {
            const Icon = activity.icon
            const colorClass =
              activity.color === "violet" ? "bg-violet" : activity.color === "green" ? "bg-green" : "bg-yellow"
            ...
            return (
              ...
              >
                <div className={`${colorClass} w-16 h-16 rounded-xl flex items-center justify-center mb-4 mx-auto`}>
                  <Icon className="w-8 h-8 text-white" />
                </div>
                <h3 className="text-lg font-bold text-deep-blue text-center mb-2">{activity.title}</h3>
                <p className="text-sm text-deep-blue/70 text-center">{activity.description}</p>
              </div>
            )
          })}
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          <Button size="lg" className="bg-violet hover:bg-violet/90 text-white font-semibold" asChild>
